...
def PriorityP(process, n):
    ...
    #Phase 2 - Select process to execute
        ...
        if current == -1:
            ...
            if len(Ready) != 0:
                currentPriority, current, processorLeft = Ready[0]
            ...
            if len(Ready) > 0 and currentPriority > Ready[0][0]:
                heapq.heappush(Ready, [currentPriority, current, processorLeft])
                currentPriority, current, processorLeft = heapq.heappop(Ready)
                if process[current]["responseTime"] == -1:
                    process[current]["responseTime"] = time-process[current]["arrivalTime"]
