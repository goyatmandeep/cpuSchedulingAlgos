#Order to enter ready state - New process > Input > Output > Just Finished
def RoundRobin(process, n, quanta):
    Ready = []
    InputQ = []
    OutputQ = []
    ...
    #Phase 2 - Select process to execute
        ...
        if current != -1:
            ...
            if processerLeft == 0:
                ...
                #context switch
                Ready.append([current, processerLeft])
                current = -1
                tempOperation = None
            else:
                tempOperation = None
